	...
	if(kplex.size() < UB) {
		...
		for(ui i = 0;i < n&&m&&kplex.size() < UB;i ++) {
			...
			if(true){
				...
				do{
					pre_size=kplex.size();
					extract_subgraph_and_prune(u, ids, ids_n, rid, vp, Qe, t_degree, exists, pend, peel_sequence, edgelist_pointer);
					if(ids_n) {
...
void Graph::extract_subgraph_and_prune(ui u, ui *ids, ui &ids_n, ui *rid, vector<pair<ui, ui> > &vp, ui *Q, ui* degree, char *exists, ept *pend, ui *peel_sequence, ui *edgelist_pointer) {
	vp.clear();
	ids_n = 0; ids[ids_n++] = u; exists[u] = 1;
	ui u_n = pstart[u];
	for(ept i = pstart[u];i < pend[u];i ++) if(peel_sequence[u]<peel_sequence[edges[i]]) {
		// edges[u_n] = edges[i]; edgelist_pointer[u_n++] = edgelist_pointer[i];
		ui v = edges[i];
	...
	for(ui i = 1;i < ids_n;i ++) {
		...
		for(ept j = pstart[v];j < pend[v];j ++) {
			// it's not degree, rather triangles... 
	...
	for(ui i = 1;i < nr_size;i ++) if(exists[ids[i]] == 2) {
		ui v = ids[i];
		for(ept j = pstart[v];j < pend[v];j ++) {
			if(!exists[edges[j]]&&peel_sequence[u]<peel_sequence[edges[j]]) {
				ids[ids_n++] = edges[j];
				exists[edges[j]] = 3;
				degree[edges[j]] = 1;
			}
