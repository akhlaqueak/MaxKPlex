...
public:
    ...
    Graph &operator=(const Graph &other)
    {
        ...
        m = other.m;
        d = new ui[n];
        memcpy(d, other.d, sizeof(ui) * n);
        edge_to = new ui[m];
        memcpy(edge_to, other.edge_to, sizeof(ui) * m);
        pstart = new ui[n + 1];
        memcpy(pstart, other.pstart, sizeof(ui) * (n + 1));
        map_refresh_id = other.map_refresh_id;
        return *this;
    }
    /**
    ...
    /**
     * @return true if G[s] is a k-plex
     */
    bool is_kPlex(set<ui> &s, int k)
    {
        map<ui, int> deg;
        for (ui u : s)
        {
