...
void Graph::kPlex_exact() {
	...
	if(kplex.size() < UB) {
		...
		kplex_solver->allocateMemory(max_n);
...
// #define FORWARD
	...
	printf(">>%s \tMaxKPlex_Size: %lu t_Total: %f t_Seesaw: %f\n", dir.substr(dir.find_last_of("/")+1).c_str(), kplex.size(), t.elapsed()/1000000.0, seesaw.ticktock());
...
ui Graph::extract_subgraph(ui u, ui *ids, ui &ids_n, ui *rid, vector<pair<ui, ui> > &vp, char *exists, ept *pstart, ept *pend, ui *edges, char *deleted, ui *edgelist_pointer) {
	ids_n = 0; vp.clear();
...
ui Graph::extract_subgraph_and_prune(ui u, ui *ids, ui &ids_n, ui *rid, ui* p_rid, vector<pair<ui, ui> > &vp, ui *Q, ui* degree, char *exists, ept *pend, char *deleted, ui *edgelist_pointer) {
	...
	for(ui i = old_nr_size;i < ids_n;i ++) {
		if(degree[ids[i]] + 2*K <= kplex.size()+2 or p_rid[u]> p_rid[ids[i]]) exists[ids[i]] = 0;
		else ids[new_size++] = ids[i];
	}
	ids_n = new_size;
#ifndef NDEBUG
